...
use crate::hardware::boards::{Board, BoardType};
use crate::hardware::devices::Device;
use crate::hardware::devices::DeviceState;
use crate::hardware::protocols::Protocol;
...
struct PartialBoard {
    name: String,
    config: Box<dyn BoardType>,
    protocol: Box<dyn Protocol>,
}

pub fn register_board_events(socket: &SocketRef) {
    socket.on("board:list", |ack: AckSender| {
        debug!("Event received: [board:list]");
        ack.send(Ack::from(Board::list())).ok();
    });
    ...
    socket.on(
        "board:create",
        |socket: SocketRef, TryData(new_board): TryData<PartialBoard>, ack: AckSender| {
            ...
            let board = match new_board {
                Ok(new_board) => {
                    let board = Board {
                        id: 0,
                        name: new_board.name,
                        config: new_board.config,
                        protocol: new_board.protocol,
                        devices: HashMap::<HardwareId, Device>::new(),
                        connected: false,
                    };
                    board.save()
                }
